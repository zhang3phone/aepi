...
export interface ImageSizeDimensions {
  width: number
  height: number
}
...
export function parseImageSizeValue(size: string): ImageSizeDimensions | null {
...
export function formatImageSizeValue(size: ImageSizeDimensions) {
  return `${Math.trunc(size.width)}x${Math.trunc(size.height)}`
}

export function normalizeStoredImageSize(size: string) {
  const trimmed = size.trim()
  if (trimmed === 'auto') return 'auto'

  const parsed = parseImageSizeValue(trimmed)
  return parsed ? formatImageSizeValue(parsed) : ''
}

export function getModelRequestImageSize(size: string) {
  const stored = normalizeStoredImageSize(size)
  if (!stored || stored === 'auto') return stored
  return normalizeImageSize(stored)
}
