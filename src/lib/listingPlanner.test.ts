...
describe('callAmazonPlannerApi', () => {
  it('uses Responses API planning with JSON schema and attached reference images', async () => {
    ...
    expect(body.instructions).toContain('Target audience and usage scene inference rule')
    expect(body.instructions).toContain('product.audience and product.scene are high-priority planning fields')
    ...
    expect(body.text.format.schema.properties.product.properties).toHaveProperty('scene')
    expect(body.text.format.schema.properties.product.required).toContain('audience')
    expect(body.text.format.schema.properties.product.required).toContain('scene')
  ...
  it('keeps size chart plans dedicated to dimensions only', async () => {
    const apiPayload = createApiPayload()
    apiPayload.imagePlans[2] = {
      ...apiPayload.imagePlans[2],
      label: 'Size Options 尺寸图',
      planMarkdown: '## 尺寸图\n\n展示不同尺寸。',
      prompt: 'Create an Amazon size options chart with product dimensions.',
      negativePrompt: 'logo, watermark',
    }
    const fetchMock = vi.fn<(input: RequestInfo | URL, init?: RequestInit) => Promise<Response>>(async () => new Response(JSON.stringify({
      output_text: JSON.stringify(apiPayload),
    }), {
    ...
    const body = JSON.parse(String(init?.body))
    expect(body.instructions).toContain('Size chart / dimension image rule')
    expect(body.instructions).toContain('make that slot a dedicated size-only image')
    expect(result.plans[2].planMarkdown).toContain('尺寸图规则')
    expect(result.plans[2].prompt).toContain('Size chart only:')
    expect(result.plans[2].prompt).toContain('Do not include usage scenarios')
    expect(result.plans[2].negativePrompt).toContain('usage scenarios')
    expect(result.plans[2].negativePrompt).toContain('product functions')
  })

  it('deduplicates repeated benefits in small icon rows', async () => {
    const apiPayload = createApiPayload()
    apiPayload.imagePlans[3] = {
      ...apiPayload.imagePlans[3],
      label: '功能小图标',
      planMarkdown: '## 小图标卖点\n\n底部放 4 个功能图标。',
      prompt: 'Create an Amazon secondary image with a bottom feature icon row showing Easy to Clean, Machine Washable, Quick-Dry, and Daily Use.',
      negativePrompt: 'logo, watermark',
    }
    const fetchMock = vi.fn<(input: RequestInfo | URL, init?: RequestInit) => Promise<Response>>(async () => new Response(JSON.stringify({
      output_text: JSON.stringify(apiPayload),
    ...
    const init = fetchMock.mock.calls[0]![1]
    const body = JSON.parse(String(init?.body))
    expect(body.instructions).toContain('Small benefit icon rule')
    expect(body.instructions).toContain('each benefit must appear only once')
    expect(result.plans[3].planMarkdown).toContain('小图标规则')
    expect(result.plans[3].prompt).toContain('Small benefit icon rule:')
    expect(result.plans[3].prompt).toContain('Do not repeat the same selling point')
    expect(result.plans[3].negativePrompt).toContain('duplicate benefit icons')
    expect(result.plans[3].negativePrompt).toContain('repeated selling point labels')
  })

  it('keeps product scene images free of text and callouts', async () => {
    const apiPayload = createApiPayload()
    apiPayload.imagePlans[4] = {
      ...apiPayload.imagePlans[4],
      label: '无文字产品场景图',
      planMarkdown: '## 场景图\n\n展示产品在真实家庭环境中的自然使用画面。',
      prompt: 'Create a realistic product scene image in a home environment with the product as the hero.',
      negativePrompt: 'logo, watermark',
    }
    const fetchMock = vi.fn<(input: RequestInfo | URL, init?: RequestInit) => Promise<Response>>(async () => new Response(JSON.stringify({
      output_text: JSON.stringify(apiPayload),
    ...
    const init = fetchMock.mock.calls[0]![1]
    const body = JSON.parse(String(init?.body))
    expect(body.instructions).toContain('Text-free product scene image rule')
    expect(body.instructions).toContain('include 1-2 product scene / lifestyle images with no on-image text')
    expect(result.plans[4].planMarkdown).toContain('无文字场景图规则')
    expect(result.plans[4].prompt).toContain('Text-free product scene image:')
    expect(result.plans[4].prompt).toContain('must contain no visible text')
    expect(result.plans[4].negativePrompt).toContain('captions')
    expect(result.plans[4].negativePrompt).toContain('feature labels')
  })
