...
export function normalizeApiProfile(input: unknown, fallback?: Partial<ApiProfile>, customProviderIds = new Set<string>()): ApiProfile {
  ...
  return {
    ...
    name: typeof record.name === 'string' && record.name.trim()
      ? record.name === 'AI策划' ? 'AI生成提示词' : record.name
      : defaults.name,
    provider,
...
export function validateApiProfile(profile: ApiProfile): string | null {
  ...
  const serverSideKeyAvailable = profile.provider === 'openai' && shouldUseApiProxy(profile.apiProxy, proxyConfig)
  if (!serverSideKeyAvailable && !profile.apiKey.trim()) return '缺少 API Key'
...
function isDefaultAmazonPlannerProfile(profile: ApiProfile): boolean {
  return profile.id === DEFAULT_AMAZON_PLANNER_PROFILE_ID &&
    (profile.name === 'AI生成提示词' || profile.name === 'AI策划') &&
    profile.provider === 'openai' &&
    profile.baseUrl === DEFAULT_BASE_URL &&
    profile.apiKey === '' &&
