...
function appendProxyTarget(path: string, target: string): string {
  if (!target) return path
  const separator = path.includes('?') ? '&' : '?'
  return `${path}${separator}__target=${encodeURIComponent(target)}`
}
...
export function buildApiUrl(
  ...
  if (useApiProxy) {
    return appendProxyTarget(`${proxyConfig?.prefix ?? DEFAULT_PROXY_PREFIX}/${apiPath}`, normalizedBaseUrl)
  }
