...
import { ChevronDownIcon, CloseIcon, CopyIcon, PlusIcon, TrashIcon, ExportIcon, ImportIcon, DragHandleIcon, LinkIcon, RefreshIcon } from './icons'
...
const DEEPSEEK_PLANNER_BASE_URL = 'https://api.deepseek.com'
const DEEPSEEK_PLANNER_MODEL = 'deepseek-v4-flash'
const LOCAL_PLANNER_BASE_URL = 'http://127.0.0.1:11434/v1'
const LOCAL_PLANNER_MODEL = 'qwen2.5vl:7b'
const DEEPSEEK_PLANNER_NOTICE = '当前 AI生成提示词配置为 DeepSeek 官方接口。DeepSeek 生成提示词阶段不会读取参考图，系统会仅用 Listing 文本和你填写的商品信息生成提示词方案；参考图仍会在正式生图时随生图请求发送。请把产品颜色、形状、结构、配件、Logo、套装数量等关键特征写进 Listing 或商品信息中。'
const LOCAL_PLANNER_NOTICE = '当前 AI生成提示词配置为本地 OpenAI 兼容模型。请确认本地服务已开放 /v1/chat/completions，并允许浏览器访问；默认 API Key 为 local，占位用，可按你的本地网关要求修改。'
...
function getComparableBaseUrl(value: string) {
  return stripTrailingSlash(value).toLowerCase()
}

function isLocalPlannerProfile(profile: Pick<ApiProfile, 'provider' | 'baseUrl' | 'apiMode'>): boolean {
  if (profile.provider !== 'openai' || profile.apiMode !== 'chat') return false
  ...
  try {
    ...
    return /^(?:https?:\/\/)?(?:localhost|127\.0\.0\.1|10\.|192\.168\.|172\.(?:1[6-9]|2\d|3[0-1])\.)/i.test(rawBaseUrl)
  }
}
...
function formatStatCount(value: number) {
  return new Intl.NumberFormat('zh-CN').format(Math.max(0, Math.trunc(value || 0)))
}

function formatStatsUpdatedAt(value: string) {
...
export default function SettingsModal() {
  ...
  const [generationStats, setGenerationStats] = useState<GenerationStatsSummary | null>(null)
  const [isLoadingGenerationStats, setIsLoadingGenerationStats] = useState(false)
  ...
  const setDraftState = useCallback((nextDraft: AppSettings) => {
  ...
  const selectedPlannerUsesOfficialDeepSeek = selectedAmazonPlannerProfile
    ...
    : false
  const selectedPlannerUsesLocalModel = selectedAmazonPlannerProfile
    ? isLocalPlannerProfile(selectedAmazonPlannerProfile)
    : false
  const amazonPlannerProfileOptions = amazonPlannerProfiles.length
    ...
    : [{ label: '暂无 Chat/Responses 提示词配置', value: '' }]

  const wasSettingsOpenRef = useRef(false)

  const loadGenerationStats = useCallback(async () => {
    if (!isGenerationStatsAvailable()) {
      setGenerationStats(null)
      setGenerationStatsError('当前没有连接本机统计后端，请使用桌面启动 BAT 后再查看。')
      return
    }

    setIsLoadingGenerationStats(true)
    setGenerationStatsError(null)
    try {
      setGenerationStats(await fetchGenerationStats())
    } catch (error) {
      setGenerationStatsError(error instanceof Error ? error.message : String(error))
    } finally {
      setIsLoadingGenerationStats(false)
    }
  }, [])
  ...
  }, [apiProxyAvailable, apiProxyLocked, showSettings, settings, reusedTaskApiProfileId, setDraftState])
  ...
  }, [activeTab, loadGenerationStats, showSettings])
  ...
  const commitSettings = (nextDraft: AppSettings) => {
    const normalizedProfiles = nextDraft.profiles.map((profile) => {
      const normalizedBaseUrl = profile.provider === 'fal'
        ? stripTrailingSlash(profile.baseUrl) || DEFAULT_FAL_BASE_URL
        : isOfficialDeepSeekPlannerProfile(profile)
          ? stripTrailingSlash(profile.baseUrl) || DEEPSEEK_PLANNER_BASE_URL
  ...
  const getDraftWithActiveProfilePatch = (patch: Partial<ApiProfile>) => ({
      ...draftRef.current,
      profiles: draftRef.current.profiles.map((profile) => profile.id === activeProfile.id ? { ...profile, ...patch } : profile),
    })
  ...
  const getLatestDraftWithNormalizedTimeout = () => {
    ...
    const latestActiveProfile = latestDraft.profiles.find((profile) => profile.id === latestDraft.activeProfileId) ?? latestDraft.profiles[0] ?? activeProfile
    const nextDraft = {
      ...latestDraft,
      profiles: isOpenAICompatibleProvider(latestDraft, latestActiveProfile.provider)
        ? latestDraft.profiles.map((profile) =>
            profile.id === latestActiveProfile.id ? { ...profile, timeout: normalizedTimeout } : profile,
          )
        : latestDraft.profiles,
    }
    return { nextDraft, normalizedTimeout }
  }

  const saveCurrentSettings = (withToast = false) => {
    const { nextDraft, normalizedTimeout } = getLatestDraftWithNormalizedTimeout()
    setTimeoutInput(String(normalizedTimeout))
    commitSettings(nextDraft)
    if (withToast) showToast('保存成功', 'success')
  }
  ...
  const createPlannerPresetProfile = (preset: 'deepseek' | 'local') => {
    setReusedTaskApiProfile(null)
    const presetConfig = preset === 'deepseek'
      ? {
          name: 'DeepSeek 生成提示词',
          baseUrl: DEEPSEEK_PLANNER_BASE_URL,
          apiKey: '',
          model: DEEPSEEK_PLANNER_MODEL,
        }
      : {
          name: '本地模型生成提示词',
          baseUrl: LOCAL_PLANNER_BASE_URL,
          apiKey: 'local',
          model: LOCAL_PLANNER_MODEL,
        }

    const existingProfile = draft.profiles.find((profile) =>
      profile.provider === 'openai' &&
      profile.apiMode === 'chat' &&
      getComparableBaseUrl(profile.baseUrl) === getComparableBaseUrl(presetConfig.baseUrl) &&
      profile.model.trim() === presetConfig.model,
    )
    ...
    if (existingProfile) {
      commitSettings({
        ...
        amazonPlannerProfileId: existingProfile.id,
      })
      setShowProfileMenu(false)
      return
    }
    ...
    const profile = createDefaultOpenAIProfile({
      id: newId(preset === 'deepseek' ? 'deepseek-planner' : 'local-planner'),
          ...
          <div className="w-full sm:w-48 shrink-0 flex flex-col border-b sm:border-b-0 sm:border-r border-gray-100 dark:border-white/[0.08] bg-gray-50/50 dark:bg-white/[0.02]">
            <nav className="flex-1 overflow-x-auto sm:overflow-y-auto custom-scrollbar p-3 space-x-1 sm:space-x-0 sm:space-y-1 flex sm:flex-col">
              ...
              <button
                ...
                className={`whitespace-nowrap flex-shrink-0 flex items-center gap-2.5 px-3 py-2.5 text-sm rounded-xl transition-colors ${activeTab === 'changelog' ? 'bg-white dark:bg-white/[0.08] shadow-sm text-blue-600 dark:text-blue-400 font-medium' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100/80 dark:hover:bg-white/[0.04]'}`}
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex-1 flex flex-col min-w-0 min-h-0 bg-transparent relative overflow-hidden">
            ...
            {activeTab === 'api' && (
              ...
              <div className="rounded-2xl border border-blue-100 bg-blue-50/70 p-3 dark:border-blue-400/20 dark:bg-blue-400/10">
                <div className="mb-2 flex items-center justify-between gap-2">
                  <span className="text-sm font-semibold text-blue-900 dark:text-blue-100">AI生成提示词配置</span>
                ...
                <div className="mt-3 grid gap-2 sm:grid-cols-2">
                  <button
                    type="button"
                    onClick={() => createPlannerPresetProfile('deepseek')}
                    className="rounded-xl border border-blue-200/70 bg-white/80 px-3 py-2 text-left text-xs font-semibold text-blue-800 transition hover:border-blue-300 hover:bg-white dark:border-blue-400/20 dark:bg-gray-950/40 dark:text-blue-100 dark:hover:border-blue-400/40"
                  >
                    添加 DeepSeek 生成提示词
                    <span className="mt-0.5 block text-[11px] font-normal text-blue-700/75 dark:text-blue-200/75">
                      Chat · {DEEPSEEK_PLANNER_MODEL}
                    </span>
                  </button>
                  <button
                    type="button"
                    onClick={() => createPlannerPresetProfile('local')}
                    className="rounded-xl border border-blue-200/70 bg-white/80 px-3 py-2 text-left text-xs font-semibold text-blue-800 transition hover:border-blue-300 hover:bg-white dark:border-blue-400/20 dark:bg-gray-950/40 dark:text-blue-100 dark:hover:border-blue-400/40"
                  >
                    添加本地模型
                    <span className="mt-0.5 block text-[11px] font-normal text-blue-700/75 dark:text-blue-200/75">
                      OpenAI 兼容 · {LOCAL_PLANNER_MODEL}
                    </span>
                  </button>
                </div>
                <div data-selectable-text className="mt-2 text-xs leading-relaxed text-blue-800 dark:text-blue-200">
                  只用于首页 Amazon 面板的 AI生成提示词；普通生图只接受当前配置为 Images API。DeepSeek 和本地模型用于生成图片方案，生图仍建议单独使用 Images API 配置。
                </div>
                ...
                {selectedPlannerUsesLocalModel && (
                  <div data-selectable-text className="mt-2 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs leading-relaxed text-emerald-800 dark:border-emerald-400/25 dark:bg-emerald-400/10 dark:text-emerald-100">
                    {LOCAL_PLANNER_NOTICE}
                  </div>
                )}
              </div>
              ...
              <div className="block">
                ...
                <div className="relative">
                  <input
                    ...
                    disabled={apiProxyEnabled}
                    placeholder={apiProxyEnabled ? '由服务器代理提供' : activeProfile.provider === 'fal' ? 'FAL_KEY' : 'sk-...'}
                    className={`w-full rounded-xl border border-gray-200/70 bg-white/60 px-3 py-2.5 pr-10 text-sm text-gray-700 outline-none transition focus:border-blue-300 dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-gray-200 dark:focus:border-blue-500/50 ${apiProxyEnabled ? 'cursor-not-allowed opacity-60' : ''}`}
                  />
                  <button
                    type="button"
              ...
              <label className="block">
                ...
                <div data-selectable-text className="mt-1.5 text-xs text-gray-500 dark:text-gray-500">
                  {activeProfile.provider === 'fal' ? (
                    ...
                    <>Responses API 用于 Agent 或 AI生成提示词等文本/多模态流程；普通生图请切换到 Images API 配置。</>
                    ...
                    isOpenRouterImageGenerationProfile(activeProfile) ? (
                      ...
                      <>Chat Completions 用于 AI生成提示词文本模型，默认 <code className="rounded bg-gray-100 px-1 py-0.5 dark:bg-white/[0.06]">{DEFAULT_CHAT_MODEL}</code>；普通生图请使用 Images API 配置。</>
                    )
                  ) : (
              ...
              <div className="pt-2">
                <button
                  ...
                  className="flex w-full items-center justify-center rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm shadow-blue-600/20 transition hover:bg-blue-500 active:scale-[0.99] dark:bg-blue-500 dark:hover:bg-blue-400"
                >
                  保存
                </button>
                <div data-selectable-text className="mt-2 text-center text-xs text-gray-500 dark:text-gray-500">
                  保存后 API 配置会保存在当前浏览器本地。
                </div>
              </div>
            </div>
            )}
            ...
            {activeTab === 'data' && (
              <div className="space-y-4">
                ...
                <div className="rounded-2xl border border-blue-100/70 bg-blue-50/40 p-4 shadow-sm dark:border-blue-500/15 dark:bg-blue-500/5">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <div className="flex items-center gap-2">
                        <svg className="h-4 w-4 text-blue-500 dark:text-blue-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3v18h18M7 15l3-3 3 2 5-7" />
                        </svg>
                        <h4 className="text-sm font-bold text-gray-900 dark:text-gray-100">生成计数</h4>
                      </div>
                      <p className="mt-1 text-xs leading-5 text-gray-500 dark:text-gray-400">
                        统计本机后端记录的成功生图张数，本地和局域网访问会合并计算，方便按天、周、月核算图片成本。
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={() => void loadGenerationStats()}
                      disabled={isLoadingGenerationStats}
                      className="inline-flex items-center gap-1.5 rounded-xl border border-blue-100 bg-white px-3 py-2 text-xs font-semibold text-blue-600 shadow-sm transition hover:bg-blue-50 disabled:opacity-60 dark:border-blue-500/20 dark:bg-white/[0.05] dark:text-blue-300 dark:hover:bg-blue-500/10"
                    >
                      <RefreshIcon className={`h-3.5 w-3.5 ${isLoadingGenerationStats ? 'animate-spin' : ''}`} />
                      刷新统计
                    </button>
                  </div>
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
                    {[
                      { label: '今日', value: generationStats?.today.count ?? 0, hint: generationStats?.today.key ?? '今天' },
                      { label: '本周', value: generationStats?.thisWeek.count ?? 0, hint: generationStats?.thisWeek.key ?? '本周' },
                      { label: '本月', value: generationStats?.thisMonth.count ?? 0, hint: generationStats?.thisMonth.key ?? '本月' },
                      { label: '累计', value: generationStats?.total ?? 0, hint: '全部记录' },
                    ].map((item) => (
                      <div key={item.label} className="rounded-xl border border-white/70 bg-white/80 p-3 shadow-sm dark:border-white/[0.06] dark:bg-white/[0.04]">
                        ...
                        <div className="mt-1 text-2xl font-bold tabular-nums text-gray-900 dark:text-gray-100">
                          ...
                          <span className="ml-1 text-xs font-semibold text-gray-400 dark:text-gray-500">张</span>
                        </div>
                        <div className="mt-1 truncate text-[11px] text-gray-400 dark:text-gray-500">{item.hint}</div>
                      </div>
                    ))}
                  </div>

                  {generationStatsError ? (
                    <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs leading-5 text-amber-700 dark:border-amber-500/20 dark:bg-amber-500/10 dark:text-amber-200">
                      统计暂不可用：{generationStatsError}
                    </div>
                  ) : (
                    <div className="mt-3 text-xs leading-5 text-gray-500 dark:text-gray-400">
                      更新时间：{formatStatsUpdatedAt(generationStats?.updatedAt ?? '')}。只统计“成功生成并写入任务记录”的图片；上传参考图、失败任务和中间预览图不会计入。
                    </div>
                  )}
                </div>
            ...
            {activeTab === 'changelog' && (
              <div className="mx-auto max-w-3xl space-y-4">
                <div className="rounded-2xl border border-blue-100/70 bg-blue-50/50 p-4 shadow-sm dark:border-blue-500/15 dark:bg-blue-500/5">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <p className="text-xs font-medium uppercase tracking-[0.18em] text-blue-500 dark:text-blue-300">Release Notes</p>
                  ...
                  <p className="mt-3 text-sm leading-6 text-gray-600 dark:text-gray-300">
                    当天功能升级、界面调整或部署能力变化会先累计；每天晚上 9 点作为版本节点，统一更新一版并记录在这里。
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  {APP_CHANGELOG.map((entry) => (
                    <section
                      key={entry.version}
                      className="rounded-2xl border border-gray-200/70 bg-white/70 p-4 shadow-sm dark:border-white/[0.08] dark:bg-white/[0.03]"
                    >
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          <div className="flex flex-wrap items-center gap-2">
                            <h4 className="text-base font-bold text-gray-900 dark:text-gray-100">v{entry.version}</h4>
                            {entry.version === __APP_VERSION__ && (
                              <span className="rounded-full bg-green-50 px-2 py-0.5 text-[11px] font-semibold text-green-600 ring-1 ring-green-100 dark:bg-green-500/10 dark:text-green-300 dark:ring-green-500/20">
                                当前版本
                              </span>
                            )}
                          </div>
                          <p className="mt-1 text-sm font-medium text-gray-700 dark:text-gray-200">{entry.title}</p>
                        </div>
                        <time className="text-xs font-mono text-gray-400 dark:text-gray-500">{entry.date}</time>
                      </div>
                      <ul className="mt-3 space-y-2 text-sm leading-6 text-gray-600 dark:text-gray-300">
                        {entry.changes.map((change) => (
                          <li key={change} className="flex gap-2">
                            <span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-blue-400 dark:bg-blue-300" />
                            <span>{change}</span>
                          </li>
                        ))}
                      </ul>
            ...
            {activeTab === 'about' && (
              <div className="relative flex h-full min-h-[300px] flex-col items-center justify-center overflow-hidden pb-8 px-6">
                {/* 
                ...
                */}
                <a
                  href="https://ai.artcloudwork.com"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="group flex flex-col items-center outline-none"
                >
                  <div className="mb-5 flex h-[88px] w-[88px] items-center justify-center overflow-hidden rounded-full border border-gray-200/80 bg-white p-3 transition-colors group-hover:bg-gray-50 dark:border-white/[0.12] dark:bg-white dark:group-hover:bg-gray-100">
                    <img src="./ae-logo.png" alt="Anaheim Electronics" className="h-full w-full object-contain" />
                  </div>
                  <h4 className="text-[17px] font-bold text-gray-800 dark:text-gray-100">AEPI03-Stardust Memory</h4>
                </a>
                <a
                  href="https://AMZ945.COM"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="mt-1.5 text-[13px] text-gray-500 transition-colors hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300"
                >
                  @ZHANG3PHONE
                </a>
                <p className="mt-2 text-[12px] font-semibold uppercase tracking-[0.18em] text-gray-400 dark:text-gray-500">
                  Power By Anaheim Electronics
                </p>
                
                <p className="mt-8 max-w-[360px] text-center text-[13px] leading-relaxed text-gray-500 dark:text-gray-400">
                  Anaheim Electronics立志于人工智能军用科技民用化，殖民卫星弱电改造和改装，人形兵器及其电子设备设计与开发。
                </p>
              </div>
            )}
          </div>
