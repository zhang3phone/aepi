...
import { normalizeStoredImageSize } from '../lib/size'
...
export default function InputBar() {
  ...
  const submitApiValidation = useMemo(() => validateApiProfile(activeProfile), [activeProfile])
  const hasSubmitApiConfig = submitApiValidation == null
  ...
  const submitButtonAriaLabel = activeAgentIsRunning
    ...
    ? maskDraft ? '遮罩编辑' : '生成图像'
    : '请先配置 API'
  const submitTooltipText = activeAgentIsRunning ? '停止生成' : `尚未完成 API 配置${submitApiValidation ? `：${submitApiValidation}` : ''}`
  ...
  const displaySize = isFalTextToImage && params.size === 'auto'
    ...
    : normalizeStoredImageSize(params.size) || DEFAULT_PARAMS.size
  ...
  const promptTokenEstimate = useMemo(() => estimateTextTokens(visiblePrompt), [visiblePrompt])
      ...
      >
        ...
        <div ref={cardRef} className={`pointer-events-auto bg-white/70 dark:bg-gray-900/70 backdrop-blur-2xl border border-white/50 dark:border-white/[0.08] shadow-[0_8px_30px_rgb(0,0,0,0.08)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.3)] rounded-2xl sm:rounded-3xl p-3 sm:p-4 ring-1 ring-black/5 dark:ring-white/10 lg:flex lg:min-h-0 lg:flex-col lg:overflow-hidden ${desktopDockCollapsed ? 'lg:hidden' : ''}`}>
          ...
          <div className="mb-3 border-b border-slate-200/70 pb-3 dark:border-white/[0.08]">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2 text-[11px] leading-5 text-slate-500 dark:text-slate-400">
              <span className="rounded-lg bg-slate-100 px-2 py-0.5 font-semibold text-slate-700 dark:bg-white/[0.06] dark:text-slate-200">
                Token 估算 {formatTokenCount(promptTokenEstimate)}
              </span>
              <span>参考图 {inputImages.length} 张</span>
              <span>本地粗略估算，实际用量以接口返回为准。</span>
            </div>
          </div>
