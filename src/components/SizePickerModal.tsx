...
const CUSTOM_SIZE_TEXT = '自定义宽高是最终成品尺寸，可输入任意正整数，不要求 16 的倍数。\n系统会在请求模型时自动使用兼容尺寸，生成完成后再调整为你输入的尺寸。'
const RATIOS = [
  { label: '1:1', value: '1:1' },
  { label: '3:2', value: '3:2' },
  { label: '2:3', value: '2:3' },
...
function parseCustomDimensions(widthInput: string, heightInput: string): ImageSizeDimensions | null {
  if (!/^\d+$/.test(widthInput.trim()) || !/^\d+$/.test(heightInput.trim())) return null
...
export default function SizePickerModal({ currentSize, onSelect, onClose, allowAuto = true }: Props) {
  ...
  const customDimensions = useMemo(() => parseCustomDimensions(customW, customH), [customW, customH])
  ...
  const previewSize = useMemo(() => {
    ...
    if (mode === 'resolution') {
      return customDimensions ? formatImageSizeValue(customDimensions) : ''
    }
    
    return ''
  }, [mode, tier, activeRatio, customDimensions])

  const modelRequestSize = useMemo(() => {
    if (!previewSize || previewSize === 'auto') return ''
    return getModelRequestImageSize(previewSize)
  }, [previewSize])
  const sizeHintText = mode === 'resolution' && previewSize && modelRequestSize && modelRequestSize !== previewSize
    ? `模型请求会使用兼容尺寸 ${modelRequestSize}，生成完成后自动调整为你输入的 ${previewSize}。`
    : SIZE_LIMIT_TEXT
  ...
  const isClamped = useMemo(() => {
    ...
    return false
  }, [mode, ratio, customRatioClamped, modelRequestSize, previewSize])
      ...
      >
        ...
        <div className="space-y-6">
              ...
              <div className="space-y-5 animate-fade-in">
                ...
                <div className="rounded-xl border border-gray-200/80 bg-gray-50/80 p-3 text-xs text-gray-600 dark:border-white/[0.05] dark:bg-white/[0.02] dark:text-gray-400">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="whitespace-pre-line leading-relaxed">{CUSTOM_SIZE_TEXT}</div>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="rounded-2xl bg-gray-50 px-4 py-3 dark:bg-white/[0.03]">
            ...
            {mode === 'resolution' && previewSize && modelRequestSize && modelRequestSize !== previewSize && (
              <div className="mt-1 text-xs leading-5 text-gray-500 dark:text-gray-400">
                模型请求 {modelRequestSize}，最终输出 {previewSize}
              </div>
            )}
          </div>
        </div>
