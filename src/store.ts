...
import { resizeImageDataUrlToCover, validateMaskMatchesImage } from './lib/canvasImage'
...
function reportTaskGeneratedImages(task: TaskRecord, count: number) {
  if (!Number.isFinite(count) || count <= 0) return

  void reportGeneratedImages({
    taskId: task.id,
    count,
    sourceMode: task.sourceMode ?? (task.agentConversationId ? 'agent' : 'gallery'),
    workflow: task.category?.workflow,
    amazonSlot: task.category?.amazonSlot,
    apiProvider: task.apiProvider,
    apiMode: task.apiMode,
    apiModel: task.apiModel,
    apiProfileName: task.apiProfileName,
  }).catch(() => {
    // 统计失败不影响生图主流程；设置页刷新时会显示后端连接状态。
  })
}
...
export type SettingsTab = 'general' | 'agent' | 'api' | 'data' | 'changelog' | 'about'
...
function getApiRequestParams(params: TaskParams): TaskParams {
  const modelSize = getModelRequestImageSize(params.size)
  return modelSize && modelSize !== params.size ? { ...params, size: modelSize } : params
}

function getOutputMimeType(params: TaskParams) {
  if (params.output_format === 'jpeg') return 'image/jpeg'
  if (params.output_format === 'webp') return 'image/webp'
  return 'image/png'
}

function getCanvasOutputQuality(params: TaskParams) {
  if (params.output_format === 'png') return undefined
  if (params.output_compression == null || !Number.isFinite(params.output_compression)) return undefined
  return Math.min(1, Math.max(0, params.output_compression / 100))
}

async function normalizeGeneratedImagesToRequestedSize(images: string[], params: TaskParams) {
  const targetSize = parseImageSizeValue(params.size)
  if (!targetSize) return images

  return Promise.all(images.map((image) =>
    resizeImageDataUrlToCover(image, targetSize, {
      mimeType: getOutputMimeType(params),
      quality: getCanvasOutputQuality(params),
    }),
  ))
}

function applyRequestedSizeToActualParams(
  actualParamsList: Array<Partial<TaskParams> | undefined> | undefined,
  params: TaskParams,
) {
  const targetSize = parseImageSizeValue(params.size)
  if (!targetSize) return actualParamsList

  const size = formatImageSizeValue(targetSize)
  return (actualParamsList ?? []).map((actualParams) => ({ ...(actualParams ?? {}), size }))
}
...
async function completeRecoveredFalTask(task: TaskRecord, result: Awaited<ReturnType<typeof getFalQueuedImageResult>>) {
  ...
  const finalImages = await normalizeGeneratedImagesToRequestedSize(result.images, task.params)
  const actualParamsList = applyRequestedSizeToActualParams(
    await resolveImageSizeParamsList(finalImages, result.actualParamsList),
    task.params,
  )
  const outputIds: string[] = []
  for (const dataUrl of finalImages) {
  ...
  reportTaskGeneratedImages(task, outputIds.length)
...
async function executeAgentRound(
  ...
  try {
    ...
    const completeAgentImageTask = async (image: AgentApiResultImage, rawResponsePayload?: string) => {
      ...
      const completedTask = useStore.getState().tasks.find((task) => task.id === taskId) ?? latestTask
      if (completedTask) reportTaskGeneratedImages(completedTask, 1)
      return taskId
    }
    ...
    while (true) {
      ...
      for (const image of result.images) {
        ...
        attachTaskToAgentRound(task.id)
        await putTask(task)
        reportTaskGeneratedImages(task, 1)
      }
...
async function executeTask(taskId: string) {
  ...
  try {
    ...
    const apiParams = getApiRequestParams(task.params)
    ...
    const actualParamsList = taskProvider === 'fal'
      ? await resolveImageSizeParamsList(finalImages, result.actualParamsList)
      : isAsyncCustomTask
      ? await readImageSizeParamsList(finalImages)
      : result.actualParamsList
    const finalActualParamsList = applyRequestedSizeToActualParams(actualParamsList, task.params)
    const actualParams = (() => {
      ...
      const [finalActualParams] = applyRequestedSizeToActualParams([result.actualParams], task.params) ?? []
      return { ...(finalActualParams ?? {}), n: outputIds.length }
    })()
